...
mod tests {
    ...
    fn test_u8() {
        ...
        number.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [0x45]);

        let d_result = u8::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_u16() {
        ...
        let d_result = u16::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_u32() {
        ...
        let d_result = u32::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_u128() {
        ...
        let d_result = u128::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_timestamp() {
        ...
        time.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [85, 85, 55, 0]);

        let d_time = DateTime::<Local>::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_timestamp_list() {
        ...
        wrap.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [6u8, 0, 0, 0, 23, 0, 86, 0, 35, 0, 96, 0, 83]);

        let d_wrap = NLengthVector::<u16, 1>::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_array() {
        ...
        array.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, array);

        let d_array = <[u8; 7]>::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_ipv4() {
        ...
        ipv4.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [245, 67, 12, 34]);

        let d_ipv4 = Ipv4Addr::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_ipv6() {
        ...
        ipv6.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [0xfc, 0x86, 0x6e, 0x01, 0x20, 0x4f, 0x49, 0x8a, 0x33, 0xcf, 0xb3, 0x0a, 0x61, 0x71, 0xe7, 0x4f]);

        let d_ipv6 = Ipv6Addr::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_ipaddr() {
        ...
        ip.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [4, 4, 227, 82, 127, 3]);

        let d_ip = IpAddr::bin_deserialise_from(buffer.as_slice()).unwrap();
    ...
    fn test_string() {
        ...
        string.bin_serialise_into(& mut buffer).unwrap();

        assert_eq!(buffer, [97, 98, 99, 100, 101, 102, 103, 0]);

        let d_string = String::bin_deserialise_from(buffer.as_slice()).unwrap();
